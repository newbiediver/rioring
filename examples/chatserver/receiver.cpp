...
void event_receiver::on_accept( tcp_socket_ptr &new_socket ) {
    ...
    new_socket->set_receive_event( []( auto &&obj, auto &&buf ) {
        event_receiver::on_receive( std::forward< decltype( obj ) >( obj ), std::forward< decltype( buf ) >( buf ) );
    } );
...
void event_receiver::on_receive( socket_ptr &obj, io_buffer *buf ) {
    ...
    while ( true ) {
        if ( !buf->read( &picked[0], sizeof( scheme ) ) ) break;
        if ( !buf->read( &picked[0], sc->size ) ) break;
        buf->pop( sc->size );
